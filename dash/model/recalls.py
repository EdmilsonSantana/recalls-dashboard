...
class Recalls(object):
    def __init__(self) -> None:
        self.df = pd.read_csv('data/recalls.csv',
    ...
    def affected_units_count(self) -> str:
        df_affected_units = self.get_data(
        )[['RECALL_ID', 'AFFECTED_UNITS']].drop_duplicates()
    ...
    def filter_by(self, time_range: tuple, manufacturer: str) -> None:
        ...
        self.filtered = self.df[self.df['REPORTED_DATE'].between(
            start_date, end_date)]

        if (manufacturer != '-'):
            self.filtered = self.filtered[self.filtered['MANUFACTURER']
                                          == manufacturer]

    def get_data(self) -> pd.DataFrame:
        return self.filtered if self.filtered is not None else self.df

    def get_recalls_grouped_by_component_category(self) -> pd.Series:
        return self.get_data()[['RECALL_ID', 'COMPONENT_CATEGORY']].drop_duplicates()['COMPONENT_CATEGORY'].value_counts()

    def get_recalls_by_vehicle_and_component_category(self) -> pd.DataFrame:
        recalls = self.get_data()[['RECALL_ID', 'VEHICLE_MODEL', 'COMPONENT_CATEGORY']].drop_duplicates()
        return recalls.groupby(['VEHICLE_MODEL', 'COMPONENT_CATEGORY']).count().reset_index()

    def get_recalls_by_vehicle_and_year(self) -> pd.DataFrame:
        recalls = self.get_data()[['RECALL_ID', 'VEHICLE_MODEL', 'VEHICLE_YEAR']].drop_duplicates()
        return recalls.groupby(['VEHICLE_MODEL', 'VEHICLE_YEAR']).count().reset_index()
    ...
    def get_affected_units_grouped_by_month_and_year(self):
        ...
        reported_period = reported_recalls['REPORTED_DATE'].dt.to_period("M")
        return reported_recalls[['AFFECTED_UNITS', 'REPORTED_DATE']].groupby(reported_period).sum(numeric_only=True)['AFFECTED_UNITS']
    ...
    def get_recalls_by(self, by: str, reported_date: datetime, limit: int):
        recalls = self.filter_recalls_by_reported_date(
            reported_date)[[by, 'RECALL_ID']].drop_duplicates()
